...
def automation_task_stmix(inst, win_x, win_y, win_w, win_h, play_duration_range=(30, 30), round_budget_seconds=180):
    """BigShare 任务流程 - 基于 CDP 的严格播放检测与累计计时"""
    print(f"🚀 [Win {inst.index}] 开始 stmix 任务 (Target: 40289/e)...")
    ...
    target_urls = ['https://stmix.io/v/wYWvNDvgcHAFB',
                   'https://stmix.io/v/6FmIdgSDtHJt0',
                   'https://stmix.io/v/1GV4tRd6CWzxQ',
                   'https://stmix.io/v/snDKfv79c93uK',
                   'https://stmix.io/v/dlNbTIDA5MJFn',
                   'https://stmix.io/v/ZdNviQB2nIHGl',
                   'https://stmix.io/v/B0znOW6bOW6UG',
                   'https://stmix.io/v/ej3HN10yEuX5s']
    ...
    def check_play_status_by_html():
        """检测 jwplayer 播放状态 - 通过 class 属性判断（支持 iframe）
        返回: (is_playing, node_id) - is_playing=True 表示正在播放，node_id 用于后续点击
        """
        try:
            # 获取完整 DOM 树，包括 iframe 内容
            doc_res = inst.send_cdp("DOM.getDocument", {"depth": -1, "pierce": True})
            root_id = doc_res.get('result', {}).get('root', {}).get('nodeId')
            if not root_id:
                print(f"[Win {inst.index}] ❌ 无法获取 root_id")
                return None, None

            # 第一步：查找 iframe 元素
            iframe_res = inst.send_cdp("DOM.querySelector", {
                "nodeId": root_id,
                "selector": "iframe[src*='streamix.so']"
            })
            iframe_id = iframe_res.get('result', {}).get('nodeId')
            
            if not iframe_id or iframe_id <= 0:
                print(f"[Win {inst.index}] ❌ 未找到 streamix iframe 元素")
                return None, None
            
            print(f"[Win {inst.index}] ✅ 找到 iframe (NodeId={iframe_id})")
            
            # 第二步：获取 iframe 的 contentDocument
            iframe_doc_res = inst.send_cdp("DOM.describeNode", {
                "nodeId": iframe_id,
                "depth": -1,
                "pierce": True
            })
            
            content_doc = iframe_doc_res.get('result', {}).get('node', {}).get('contentDocument')
            # print('content_doc',content_doc)
            if not content_doc:
                print(f"[Win {inst.index}] ❌ 无法获取 iframe contentDocument")
                return None, None
            
            iframe_root_id = content_doc.get('nodeId')
            print(f"[Win {inst.index}] ✅ 获取到 iframe 内部文档 (NodeId={iframe_root_id})")
            
            # 第三步：在 iframe 内查找 jwplayer
            jwplayer_res = inst.send_cdp("DOM.querySelector", {
                "nodeId": iframe_root_id,
                "selector": "#jwplayer"
            })
            print('jwplayer_res',jwplayer_res)
            node_id = jwplayer_res.get('result', {}).get('nodeId')
            
            if not node_id or node_id <= 0:
                print(f"[Win {inst.index}] ❌ 在 iframe 内未找到 #jwplayer 元素")
                return None, None
            
            print(f"[Win {inst.index}] ✅ 在 iframe 内找到 #jwplayer (NodeId={node_id})")

            # 先获取 jwplayer 元素的 HTML，检测当前播放状态
            html_res = inst.send_cdp("DOM.getOuterHTML", {"nodeId": node_id})
            if 'result' not in html_res or 'outerHTML' not in html_res['result']:
                print(f"[Win {inst.index}] ❌ 无法获取元素 HTML")
                return None, None

            play_html = html_res['result']['outerHTML']
            
            # 通过 class 属性判断播放状态
            # jw-state-playing = 播放中
            if 'jw-state-playing' in play_html:
                is_play = True
                print(f"[Win {inst.index}] 🎬 检测到播放状态: jw-state-playing (无需点击)")
                return is_play, node_id
            
            # 未播放，需要点击 jwplayer 元素
            print(f"[Win {inst.index}] ⏸️ 视频未播放，准备点击 #jwplayer 元素...")
            box_res = inst.send_cdp("DOM.getBoxModel", {"nodeId": node_id})
            if 'result' in box_res and 'model' in box_res.get('result', {}):
                ...
                if model['width'] > 0 and model['height'] > 0:
                    ...
                    print(f"[Win {inst.index}] 📍 点击位置: ({int(cx)}, {int(cy)})")
                    ...
                    print(f"[Win {inst.index}] ✅ 已点击 #jwplayer 元素")
                    
                    # 等待 3 秒让视频开始播放
                    time.sleep(3)
                    
                    # 重新获取 HTML，检测点击后的播放状态
                    html_res = inst.send_cdp("DOM.getOuterHTML", {"nodeId": node_id})
                    if 'result' in html_res and 'outerHTML' in html_res['result']:
                        play_html = html_res['result']['outerHTML']
                        if 'jw-state-playing' in play_html:
                            is_play = True
                            print(f"[Win {inst.index}] 🎬 点击后检测到播放状态: jw-state-playing")
                        else:
                            is_play = False
                            class_match = re.search(r'class="([^"]*)"', play_html)
                            if class_match:
                                print(f"[Win {inst.index}] ⚠️ 点击后仍未播放，当前 class: {class_match.group(1)}")
                    else:
                        is_play = False
                else:
                    is_play = False
            ...
            print(f"[Win {inst.index}] ❌ 检测播放状态异常: {e}")
            import traceback
            traceback.print_exc()
            return None, None
    ...
    has_ever_played = False  # 记录是否曾经播放过（用于区分网速加载和真正的暂停）
    first_play_time = None  # 记录首次开始播放的时间
    ...
    print(f"[Win {inst.index}] ⏱️ 如果 3 分钟内未开始播放，将重启线程")

    while inst.running:
        try:
            now = time.time()

            # 如果视频还未开始播放，3 分钟后重启线程
            if not has_ever_played and now - task_start > 180:
                print(f"⚠️ [Win {inst.index}] 3 分钟内未开始播放，重启线程")
                inst.send_cdp('Page.navigate', {'url': 'about:blank'})
                return False
            
            # 如果视频已开始播放，使用调整后的超时时间（从首次播放开始计算）
            if has_ever_played and first_play_time is not None:
                adjusted_timeout = round_budget_seconds + 60  # 播放后额外增加 60 秒容错时间
                if now - first_play_time > adjusted_timeout:
                    print(f"⚠️ [Win {inst.index}] 播放后任务超时 ({adjusted_timeout}s)，放弃本轮任务")
                    inst.send_cdp('Page.navigate', {'url': 'about:blank'})
                    return False
            ...
            if now - last_check >= 3.0:
                ...
                if is_play is True:
                    if play_start_time is None:
                        play_start_time = now
                        if not has_ever_played:
                            has_ever_played = True
                            first_play_time = now
                            print(f"[Win {inst.index}] ✅ 检测到视频首次开始播放")
                        else:
                            print(f"[Win {inst.index}] ✅ 视频恢复播放（加载完成）")
                    else:
                        elapsed = now - play_start_time
                        accumulated_play_time = elapsed
                        print(
                            f"[Win {inst.index}] 📊 播放中... 已累计: {accumulated_play_time:.1f}s / {required_play_duration}s")
                    ...
                    if play_start_time is not None:
                        # 如果曾经播放过，现在暂停了，说明是网速问题在加载，不需要点击
                        if has_ever_played:
                            print(f"⏳ [Win {inst.index}] 视频加载中（网速问题），等待恢复播放...")
                        else:
                            print(f"⚠️ [Win {inst.index}] 播放中断，重置计时器")
                        play_start_time = None
                        accumulated_play_time = 0.0
                    else:
                        # 只有在从未播放过的情况下才尝试点击
                        if not has_ever_played:
                            ...
                            # 曾经播放过，现在检测到暂停，不点击，只是等待
                            print(f"⏳ [Win {inst.index}] 视频暂停中（可能是加载），等待恢复...")
                else:
                    if now - last_reload > 30:
    ...
    def worker_loop(inst, wx, wy, ww, wh):
        while True:
            try:
                ...
                inst.launch()
                time.sleep(5)
                ok = automation_task_stmix(inst, wx, wy, ww, wh, PLAY_DURATION_RANGE,
